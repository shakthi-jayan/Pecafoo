...
const HomePage = () => {
    ...
    const fetchRestaurants = useCallback(async () => {
        try {
            ...
            // SAFE: Handle paginated, array, or error responses safely
            let results = [];
            ...
            setRestaurants(safeResults);
            setFeaturedRestaurants(
                safeResults.filter((restaurant) => restaurant && restaurant.is_featured === true)
            );
        } catch (error) {
    ...
    useEffect(() => {
        const loadCategories = async () => {
            try {
                const { data } = await restaurantsAPI.getPlatformCategories();
                // Ensure data is an array
                if (Array.isArray(data)) {
                    setPlatformCategories(data);
                } else if (data && Array.isArray(data.results)) {
                    setPlatformCategories(data.results);
                } else {
    ...
    // SAFE: Ensure restaurants is always an array before passing to RestaurantMap
    const safeRestaurants = Array.isArray(restaurants) ? restaurants : [];
    const safeCoords = coords && Array.isArray(coords) && coords.length === 2 ? coords : null;
    ...
    return (
        <div className="page page-shell stack-safe">
            ...
            >
                <div className="section-header">
                    <h2 className="section-title" style={{ marginBottom: 0 }}>
                        {coords ? 'Nearby Restaurants' : safeRestaurants.length > 0 ? 'Restaurants' : 'No Restaurants Yet'}
                    </h2>
                    {coords && (
                        <button
