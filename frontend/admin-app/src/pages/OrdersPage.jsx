...
const OrdersPage = () => {
    ...
    return (
        <div className="page-shell">
            <div className="page-header"><h1 className="page-title">Orders</h1><span style={{ color: 'var(--text-muted)', fontSize: '0.875rem' }}>{orders.length} total</span></div>
            ...
            <div style={{ position: 'relative', marginBottom: 16 }}>
                ...
                <input className="input" placeholder="Search by order # or restaurant..." value={search} onChange={e => setSearch(e.target.value)} style={{ paddingLeft: 36 }} />
            </div>
            ...
            <div className="card stack-safe">
                {loading ? [1, 2, 3, 4, 5].map(i => <div key={i} className="skeleton" style={{ height: 48, marginBottom: 8 }} />) :
                    filtered.length > 0 ? (
                        ...
                        <div className="empty-state"><ClipboardList /><h3>No orders found</h3></div>
                    )}
            </div>
            ...
            {selectedOrder && (
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }} onClick={() => setSelectedOrder(null)}>
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card" style={{ width: '100%', maxWidth: 520, maxHeight: '80vh', overflow: 'auto' }} onClick={e => e.stopPropagation()}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                            <h2 style={{ fontWeight: 700 }}>Order #{selectedOrder.order_number}</h2>
                        ...
                        <div style={{ marginBottom: 16 }}>
                            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: 4 }}>Restaurant: <strong style={{ color: 'var(--text-primary)' }}>{selectedOrder.restaurant_name}</strong></p>
                            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)', marginBottom: 4 }}>Customer: <strong style={{ color: 'var(--text-primary)' }}>{selectedOrder.customer_name || selectedOrder.customer_email || '—'}</strong></p>
                            <p style={{ fontSize: '0.85rem', color: 'var(--text-muted)' }}>Address: {selectedOrder.delivery_address}</p>
                        </div>

                        {}
                        <div style={{ marginBottom: 16 }}>
                            <h4 style={{ fontWeight: 600, marginBottom: 8 }}>Items</h4>
                            ...
                            <div style={{ display: 'flex', justifyContent: 'space-between', padding: '8px 0', fontWeight: 700, marginTop: 4 }}>
                                <span>Total</span><span style={{ color: 'var(--accent)' }}>₹{selectedOrder.total}</span>
                            </div>
                        </div>
                        ...
                        {!['delivered', 'cancelled'].includes(selectedOrder.status) && (
                            <div>
                                <h4 style={{ fontWeight: 600, marginBottom: 8 }}>Update Status</h4>
                        ...
                        <button onClick={() => setSelectedOrder(null)} className="btn btn-secondary" style={{ width: '100%', marginTop: 16 }}>Close</button>
                    </motion.div>
                </div>
            )}
        </div>
