...
const SettingsPage = () => {
    ...
    const Field = ({ label, name, type = 'text', formData, setFormData, ...props }) => (
        <div style={{ marginBottom: 16 }}>
            ...
            <input className="input" type={type} value={formData[name]} onChange={e => setFormData({ ...formData, [name]: e.target.value })} {...props} />
        </div>
    );
    ...
    const Toggle = ({ label, name, formData, setFormData }) => (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 0', borderBottom: '1px solid var(--border)' }}>
            ...
            <button onClick={() => setFormData({ ...formData, [name]: !formData[name] })}
    ...
    if (loading) return <div><div className="page-header"><h1 className="page-title">Settings</h1></div>{[1, 2, 3].map(i => <div key={i} className="skeleton" style={{ height: 60, marginBottom: 12 }} />)}</div>;
    ...
    return (
        <div>
                ...
                <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000, padding: 20 }}>
                    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="card" style={{ width: '100%', maxWidth: 520, maxHeight: '90vh', overflow: 'auto' }}>
                        ...
                        <form onSubmit={handleCreate}>
                            ...
                            <div style={{ marginTop: 12, marginBottom: 12, padding: 12, background: 'var(--bg-elevated)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid var(--border)' }}>
                                <div style={{ flex: 1 }}>
            ...
            {restaurants.length === 0 && !showCreate ? (
                <div className="card"><div className="empty-state"><Store /><h3>No Restaurant Yet</h3><p>Create your first restaurant to get started</p><button onClick={() => setShowCreate(true)} className="btn btn-primary" style={{ marginTop: 16 }}><Plus size={18} /> Create Restaurant</button></div></div>
            ) : selected && (
                <>
                    ...
                    <motion.div className="card" style={{ marginBottom: 16 }} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}>
                        ...
                        <div style={{ marginTop: 12, marginBottom: 16, padding: 12, background: 'var(--bg-elevated)', borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'space-between', border: '1px solid var(--border)' }}>
                            <div style={{ flex: 1 }}>
                                <div style={{ fontSize: '0.875rem', fontWeight: 600 }}>GPS Coordinates</div>
