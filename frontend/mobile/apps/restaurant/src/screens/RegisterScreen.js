...
const RegisterScreen = ({ navigation }) => {
  ...
  const handleRegister = async () => {
    ...
    setLoading(true);
    try { const data = await register({ ...form, requested_role: 'restaurant_owner' }); if (data) navigation.navigate('Verification'); }
  ...
  return (
    <SafeAreaView style={s.container}><KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={s.flex}>
      <ScrollView contentContainerStyle={s.scroll} keyboardShouldPersistTaps="handled" showsVerticalScrollIndicator={false}>
        <Text style={s.title}>Register Restaurant</Text><Text style={s.subtitle}>Create your restaurant partner account</Text>
        ...
        <Button title="Create Account" onPress={handleRegister} loading={loading} color={BRAND} style={s.btn} />
        <View style={s.footer}><Text style={s.ft}>Already have an account? </Text><TouchableOpacity onPress={() => navigation.navigate('Login')}><Text style={{ color: BRAND, fontWeight: '600', fontSize: 14 }}>Sign in</Text></TouchableOpacity></View>
      </ScrollView>
