...
def _fallback_distance_km(origin_lat: float, origin_lng: float, dest_lat: float, dest_lng: float) -> float:
    earth_radius_km = 6371
    lat1, lng1, lat2, lng2 = map(radians, [origin_lat, origin_lng, dest_lat, dest_lng])
    d_lat = lat2 - lat1
    d_lng = lng2 - lng1
...
class DeliveryPricingService:
    ...
    def get_active_pricing_config(cls) -> DeliveryPricingConfig:
        ...
        if not config:
            config = DeliveryPricingConfig(
                base_fee=Decimal("35.00"),
                per_km_rate=Decimal("8.00"),
                base_distance_km=3.0,
                min_order_fee_threshold=Decimal("199.00"),
                small_cart_fee=Decimal("20.00"),
                platform_margin_percent=18.0,
                is_active=True,
            )
        return config
    ...
    def get_active_payout_config(cls) -> PartnerPayoutConfig:
        ...
        if not config:
            config = PartnerPayoutConfig(
                base_pay=Decimal("28.00"),
                per_km_incentive=Decimal("5.00"),
                peak_hour_bonus=Decimal("10.00"),
                rain_bonus=Decimal("15.00"),
                long_distance_threshold_km=8.0,
                long_distance_bonus=Decimal("20.00"),
                is_active=True,
            )
        return config
    ...
    def get_route_distance_km(cls, origin_lat: float, origin_lng: float, dest_lat: float, dest_lng: float) -> float:
        ...
        try:
            ...
            return _fallback_distance_km(origin_lat, origin_lng, dest_lat, dest_lng)
