...
class AdminRestaurantApproveView(generics.GenericAPIView):
    """POST /api/restaurants/admin/<uuid:pk>/approve/"""
    permission_classes = [IsAuthenticated, IsAdmin]
    ...
    def post(self, request, pk):
        ...
        restaurant.save(update_fields=['approval_status', 'is_active'])
        return Response({'status': 'approved', 'id': str(restaurant.id)})


class AdminRestaurantRejectView(generics.GenericAPIView):
    """POST /api/restaurants/admin/<uuid:pk>/reject/"""
    permission_classes = [IsAuthenticated, IsAdmin]
    queryset = Restaurant.objects.all()

    def post(self, request, pk):
        restaurant = self.get_object()
        restaurant.approval_status = Restaurant.ApprovalStatus.PENDING
        restaurant.is_active = False
        reason = request.data.get('reason', '')
        restaurant.rejection_reason = reason
        save_fields = ['approval_status', 'is_active']
        if hasattr(restaurant, 'rejection_reason'):
            save_fields.append('rejection_reason')
        restaurant.save(update_fields=save_fields)
        return Response({'status': 'rejected', 'id': str(restaurant.id)})
